...
export function registerTestExecute(server: McpServer, deps: ServerDeps): void {
  server.registerTool(
    ...
    async (args, extra) => {
      // Snapshot project_root once up-front so the audit hook can tag the
      // row even when the body throws before reaching the tool-local
      // binding.
      const auditProjectRoot = readProjectRoot(deps);
      return runTool(
        async () => {
          if (!deps.projectDb) {
        ...
        (payload) => {
          // stdout/stderr tails can contain arbitrary test output; under
          // full-audit mode that would persist to disk. Blank them before
          // the hash so both the hash and any stored payload exclude
          // runtime chatter — the pattern-based redactor doesn't know
          // about our structured content shape.
          const auditOutputs =
            payload.ok && payload.content && typeof payload.content === "object"
              ? {
                  ...payload,
                  content: {
