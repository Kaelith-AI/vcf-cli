...
export async function runInit(opts: { telemetry?: boolean } = {}): Promise<void> {
  ...
  if (existsSync(cfgPath)) {
    ...
    const seed = [
      ...
      "  # personal_data.allow_list: exact-match strings that suppress ship_audit",
      "  # personal-data warnings (e.g. contributor emails in README, CODEOWNERS).",
      "  # personal_data:",
      "  #   allow_list: []",
      "",
      ...
      "",
      "# Optional: ship-release gate hardening (followup #25 items 5+6).",
      "# ship:",
      "#   # strict_chain: when true, ship_release requires a passing ship_audit",
      "#   # AND a successful ship_build within strict_chain_window_minutes.",
      "#   strict_chain: false",
      "#   strict_chain_window_minutes: 60",
      "#   # version_check: when true, ship_release rejects if the tag is not",
      "#   # semver-newer than the last release (hard gate with strict_chain,",
      "#   # soft-warn without it).",
      "#   version_check: false",
      "",
