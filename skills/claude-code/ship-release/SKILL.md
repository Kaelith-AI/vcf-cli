...
- `ship_audit` must have passed (when `config.ship.strict_chain: true`, this is enforced automatically).
- `ship_build` must have succeeded (same enforcement when strict_chain is on).
